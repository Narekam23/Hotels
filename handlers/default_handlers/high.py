...
from API_site.core import site_api, utils_func
from API_site.core import url, headers, querystring, payload_list, payload_detail
...
def city_name(message: Message) -> None:
    ...
    logger.info("Запускается команда 'high'")

    bot.set_state(message.from_user.id,
                  UserInfoState.city_name_high,
                  message.chat.id
                  )
...
@bot.message_handler(state=UserInfoState.city_name_high)
...
@bot.callback_query_handler(func=lambda call: True, state=UserInfoState.city_name_high)
def callback(call):
    """Callback функция.
    ...
    if call.message:
        ...
        bot.set_state(call.from_user.id, UserInfoState.count_adults_high)
        bot.send_message(call.message.chat.id, "Введите количество взрослых")


@bot.message_handler(state=UserInfoState.count_adults_high)
def get_adults(message: Message) -> None:
    ...
    if message.text.isdigit():
        ...
        bot.set_state(message.from_user.id,
                      UserInfoState.arrival_date_high,
                      message.chat.id
                      )
...
@bot.callback_query_handler(
    ...
    state=UserInfoState.arrival_date_high)
def callback_inline(call: CallbackQuery):
    ...
    if action == "DAY":
        ...
        bot.set_state(call.from_user.id, UserInfoState.departure_date_high)


@bot.callback_query_handler(
    func=lambda call: call.data.startswith(calendar_1_callback.prefix),
    state=UserInfoState.departure_date_high)
def callback_inline(call: CallbackQuery):
    """Функция проверяет корректность введённой даты и просит ввести
    количество отелей, которые хочет увидеть пользователь"""
    name, action, year, month, day = call.data.split(calendar_1_callback.sep)
    date = calendar.calendar_query_handler(
        bot=bot,
        call=call,
        ...
        with bot.retrieve_data(call.from_user.id) as data:
            ...
            if answer:
                ...
                bot.send_message(call.from_user.id,
                                 "Какое количество отелей показать?(макс. 5шт)"
                                 )
                bot.set_state(call.from_user.id, UserInfoState.count_hotels_high)
            else:
                now = datetime.datetime.now()
                bot.send_message(
...
@bot.message_handler(state=UserInfoState.count_hotels_high)
def search_hotels(message: Message) -> None:
    ...
    if int(message.text) <= 5 and message.text.isdigit():
        ...
        try:
            with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
                ...
                hotels_list_info = list(reversed(hotels_list_info))
            ...
            bot.send_message(message.chat.id,
                             ...
                             reply_markup=markup)
            bot.set_state(message.from_user.id,
                          UserInfoState.choice_hotel_high,
                          message.chat.id)
...
@bot.callback_query_handler(func=lambda call: True, state=UserInfoState.choice_hotel_high)
def callback(call):
    ...
    if call.message:
        ...
        try:
            ...
            bot.send_message(call.from_user.id,
                             ...
                             reply_markup=markup)
            bot.set_state(call.from_user.id, UserInfoState.photo_high)
...
@bot.callback_query_handler(func=lambda call: True, state=UserInfoState.photo_high)
def callback(call):
    ...
    with bot.retrieve_data(call.from_user.id) as data:
        ...
        try:
            ...
            logger.info("Команда 'high' завершена")
