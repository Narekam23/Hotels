...
def city_name(message: Message) -> None:
    ...
    logger.info("Запускается команда 'low'\n")

    bot.set_state(message.from_user.id,
                  UserInfoState.city_name_low,
                  message.chat.id
                  )
...
@bot.message_handler(state=UserInfoState.city_name_low)
...
@bot.callback_query_handler(func=lambda call: True, state=UserInfoState.city_name_low)
def callback(call):
    """Callback функция.
    ...
    if call.message:
        ...
        bot.set_state(call.from_user.id, UserInfoState.count_adults_low)
        bot.send_message(call.message.chat.id, "Введите количество взрослых")


@bot.message_handler(state=UserInfoState.count_adults_low)
def get_adults(message: Message) -> None:
    ...
    if message.text.isdigit():
        ...
        bot.set_state(message.from_user.id,
                      UserInfoState.arrival_date_low,
                      message.chat.id
                      )
...
@bot.callback_query_handler(
    ...
    state=UserInfoState.arrival_date_low)
def callback_inline(call: CallbackQuery):
    ...
    if action == "DAY":
        ...
        bot.set_state(call.from_user.id, UserInfoState.departure_date_low)


@bot.callback_query_handler(
    func=lambda call: call.data.startswith(calendar_1_callback.prefix),
    state=UserInfoState.departure_date_low)
def callback_inline(call: CallbackQuery):
    ...
    if action == "DAY":
        ...
        with bot.retrieve_data(call.from_user.id) as data:
            ...
            if answer:
                ...
                bot.set_state(call.from_user.id, UserInfoState.count_hotels_low)
            else:
                now = datetime.datetime.now()
                bot.send_message(
...
@bot.message_handler(state=UserInfoState.count_hotels_low)
    ...
    if int(message.text) <= 5 and message.text.isdigit():
        ...
        try:
            ...
            bot.send_message(message.chat.id,
                             ...
                             reply_markup=markup)
            bot.set_state(message.from_user.id,
                          UserInfoState.choice_hotel_low,
                          message.chat.id)
            logger.info("Функция выполнилась\n")
...
@bot.callback_query_handler(func=lambda call: True, state=UserInfoState.choice_hotel_low)
def callback(call):
    ...
    if call.message:
        ...
        try:
            ...
            bot.send_message(call.from_user.id,
                             ...
                             reply_markup=markup)
            bot.set_state(call.from_user.id, UserInfoState.photo_low)
...
@bot.callback_query_handler(func=lambda call: True, state=UserInfoState.photo_low)
def callback(call):
    ...
    with bot.retrieve_data(call.from_user.id) as data:
        ...
        try:
            ...
            logger.info("Команда 'low' завершена\n")
